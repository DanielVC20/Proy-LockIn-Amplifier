...
def Datos1(frec_1, r_1):
    ...
    def grafica_2(frec, r):
        ...
        print("mean 1 = {} E-6 V".format(np.mean(y)))
        print("std 1 = {} E-6 V".format(np.std(y)))
    ...
    def grafica_3(frec, r, frec_corte):
        ...
        y_ajuste = funcion(x_ajuste, popt[0], popt[1])
        ...
        y_pred = funcion(x, popt[0], popt[1])
        print("R^2 = {}".format(sklearn.metrics.r2_score(y, y_pred)))
        
        plt.figure()
        plt.figure(figsize=(8,4.5))
        plt.scatter(x, y - y_pred, s=15, label="$r_{experimental} - r_{ajustado}$")
        plt.xlabel(lab_x)
        plt.ylabel(lab_y)
        plt.legend()
        ...
        print("mean 2 = {} E-3 V".format(mu))
        print("std 2 = {} E-3 V".format(sigma))
        return None
...
def Datos4(frec4_in, r4_in, frec4_out, r4_out, frec_1, r_1, frec_2, r_2):    
    lab_x = "Frecuencia (MHz)"
    lab_y = "Amplitud (mV)"

    plt.figure(figsize=(8,4.5))
    plt.plot(frec4_in[1:]*1E-6, r4_in[1:]*1E3)
    plt.xlabel(lab_x)
    plt.ylabel(lab_y)
    ...
    plt.subplot(1, 2, 2)
    plt.scatter(frec4_in[ii4][1:]*1E-3, r4_in[ii4][1:]*1E3, label="Conexión +In-Top", s=15, c="tab:orange", alpha=0.3)
    plt.scatter(frec_1[ii1][1:]*1E-3, r_1[ii1][1:]*1E3, label="Puerto +In", s=15, c="tab:blue")
...
def Datos5(frec4_in, r4_in, frec4_out, r4_out, frec5_in, r5_in, frec5_out, r5_out, frec_1, r_1, frec_2, r_2):
    ...
    print("frec = {}".format(frec5_in[4]))
    print("mu = {}".format(np.mean(r5_in[4:]*1E3)))
    print("std = {}".format(np.std(r5_in[4:]*1E3)))
...
#Datos4(frec4_in, r4_in, frec4_out, r4_out, frec_1, r_1, frec_2, r_2)
